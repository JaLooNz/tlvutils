...
/**
 * The abstract BERTag class encapsulates a BER TLV tag. The rules on the allowed encoding of the
 * Tag field are based on the ASN.1 BER encoding rules of ISO/IEC 8825-1:2002.
 * <p>
...
public abstract class BERTag {
    ...
    /**
     * Create a BERTLV Tag object from the binary representation in the byte array. All
     * implementations must support tag numbers up to 0x3FFF. Note that the returned BERTag must be
     * cast to the correct subclass: PrimitiveBERTag or ConstructedBERTag to access their
     * specialized API.
     *
     ...
     * @throws ArrayIndexOutOfBoundsException if accessing the input array would cause access of
     *                                        data outside array bounds, or if the array offset parameter is negative
     ...
     */
    static BERTag getInstance(byte[] bArray, short bOff) throws ArrayIndexOutOfBoundsException, NullPointerException, TLVException {
        BERTag tag = null;
    ...
    /**
     * Writes the BER Tag bytes representing the specified tag class, constructed flag and the tag
     * number as a BER Tag representation in the specified byte array
     *
     * @param tagClass      encodes the tag class. Valid codes are the BER_TAG_CLASS_MASK_* constants
     ...
     * @throws ArrayIndexOutOfBoundsException if accessing the output array would cause access of
     *                                        data outside array bounds, or if the array offset parameter is negative
    ...
    public static short toBytes(short tagClass,
                                ...
                                short tagNumber,
                                byte[] outArray,
                                short bOff) throws ArrayIndexOutOfBoundsException, NullPointerException, TLVException {
     ...
     *                                        TLVException.MALFORMED_TAG if tag representation in the byte array is malformed
     */
    public static short size(byte[] berTagArray, short bOff) throws ArrayIndexOutOfBoundsException, NullPointerException, TLVException {
    ...
    public static short tagNumber(byte[] berTagArray,
        ...
        if ((tag & 0x1f) == 0x1f) {
            tag = 0;
            while ((berTagArray[tagOff] & 0x80) != 0) {
                tag = (short) ((tag << 7) | (berTagArray[tagOff] & 0x7F));
                tagOff++;
                if (tagOff - bOff > berTagArray.length) {
                    throw new ArrayIndexOutOfBoundsException();
                }
            }
            tag = (short) ((tag << 7) | (berTagArray[tagOff] & 0x7F));
     ...
     * @param berTagArray input byte array
     * @param bOff        offset within byte array containing first byte
     * @return the BER Tag class. One of the BER_TAG_CLASS_MASK_*.. constants defined above. See BER_TAG_CLASS_MASK_APPLICATION.
     * @throws ArrayIndexOutOfBoundsException if accessing the input array would cause access of data outside array bounds, or if the array offset parameter is negative
     * @throws NullPointerException           if berTagArray is null
     * @throws TLVException                   with the following reason codes:
     *                                        TLVException.MALFORMED_TAG if tag representation in the byte array is malformed.
     ...
     * @param berTagArray input byte array
     * @param bOff        offset within byte array containing first byte
     * @return true if input data is a well formed BER Tag structure of tag size equal to or less than the supported maximum size, false otherwise
     */
    public static boolean verifyFormat(byte[] berTagArray, short bOff) throws ArrayIndexOutOfBoundsException, NullPointerException {
        try {
    ...
    public static boolean isConstructed(byte[] berTagArray,
        ...
        boolean bitPC = ((berTagArray[bOff] & 0x20) == 0x20);
        if (bitPC)
    ...
    /**
     * Abstract init method. (Re-)Initialize this BERTag object from the binary representation in
    ...
    /**
     * Compares this BER Tag with another. Note that this method does not throw exceptions. If the
     * parameter otherTag is null, the method returns false
     *
     * @param otherTag
