...
/**
 * The Util class contains common utility functions. Some of the methods may be
 * implemented as native functions for performance reasons. All methods in Util,
 * class are static methods.
 * <p>
 * Some methods of Util, namely arrayCopy(), arrayCopyNonAtomic(), arrayFill(),
 * arrayFillNonAtomic() and setShort(), refer to the persistence of array
 * objects. The term persistent means that arrays and their values persist from
 * one CAD session to the next, indefinitely. The JCSystem class is used to
...
public class Util {
	 ...
	 * ArrayIndexOutOfBoundsException exception is thrown. If srcOff+length is
	 * greater than src.length, the length of the src array a
	 * ArrayIndexOutOfBoundsException exception is thrown. If destOff+length is
	 * greater than dest.length, the length of the dest array an
	 * ArrayIndexOutOfBoundsException exception is thrown. If src or dest parameter
	 * is null a NullPointerException exception is thrown.
	 *
	 ...
	 */
	public static final short arrayCompare(byte[] src, short srcOff, byte[] dest, short destOff, short length)
	 ...
	 * @throws ArrayIndexOutOfBoundsException if copying would cause access of data
	 ...
	 * @param bOff   offset within byte array containing first byte (the high order
	 *               byte)
	 * @return the short value the concatenated result
	 * @throws NullPointerException           if the bArray parameter is null
	 * @throws ArrayIndexOutOfBoundsException if the bOff parameter is negative or
	 *                                        if bOff+2 is greater than the length
	 *                                        of bArray
	 */
	...
	/**
	 * Deposits the short value as two successive bytes at the specified offset in
	 * the byte array.
	 *
	 * @param bArray byte array
	 * @param bOff   offset within byte array containing first byte (the high order
